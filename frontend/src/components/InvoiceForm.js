...
const InvoiceForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Validate items
      const invalidItems = formData.items.some(item => !item.name || item.count <= 0 || item.cost <= 0);
      if (invalidItems) {
        setMessage({
          type: 'error',
      ...
      const calculatedTotal = formData.items.reduce((sum, item) => {
        return sum + (parseFloat(item.count) * parseFloat(item.cost));
      }, 0);
      
      // Round to 2 decimal places
      const roundedTotal = Math.round(calculatedTotal * 100) / 100;
      
      // Create a new data object with the correct total
      ...
      console.log('Item costs:', formData.items.map(item => ({
        name: item.name,
        count: item.count,
        cost: item.cost,
